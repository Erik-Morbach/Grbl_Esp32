...
unsigned long dualHandButton[4] = {0,0};
...
void user_defined_macro(uint8_t index) {
	...
	dualHandButton[index] = millis();
	if(abs(dualHandButton[index]-dualHandButton[index^1])>DUALHAND_LIMIT)
		return;
	uint16_t waitTime = 0;
	std::vector<std::pair<uint8_t,uint8_t>> list{
